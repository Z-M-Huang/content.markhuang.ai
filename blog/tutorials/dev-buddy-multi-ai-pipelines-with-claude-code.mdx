...
[Dev Buddy](https://github.com/Z-M-Huang/vcp) is an open-source Claude Code plugin that runs **multiple AI models through structured pipelines** — requirements gathering, planning, implementation, and code reviews, each performed by different models with independent perspectives. It's part of the [VCP (Vibe Coding Protocol)](https://github.com/Z-M-Huang/vcp) project and builds on VCP's standards enforcement with multi-AI pipeline orchestration.
...
Dev Buddy is part of the [VCP project](https://github.com/Z-M-Huang/vcp) — the same repo that provides standards enforcement. VCP handles proactive rule enforcement; Dev Buddy adds multi-model review on top.
